...
func TestDeviceNetworkIdentitiesTable(t *testing.T) {
	var gotPath string
	...
	out, err := execCmd(t, "",
		"device", "network-identities", "dev1",
		"--service", "iroh", "--instance", "default",
		"--org", "acme", "--product", "thermostat", "--uri", srv.URL, "--token", "tok",
	)
	if err != nil {
		t.Fatalf("network-identities: %v\n%s", err, out)
	}

	if gotPath != "/api/orgs/acme/products/thermostat/devices/dev1/network_identities" {
...
func TestDeviceNetworkIdentitiesEmpty(t *testing.T) {
	...
	if !strings.Contains(out, "No network identities found for device dev1") {
		t.Errorf("expected empty message, got:\n%s", out)
	}
}

func TestDeviceNetworkIdentitiesJSON(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(deviceIdentitiesResponse))
	}))
	...
	if err != nil {
		t.Fatalf("network-identities json: %v\n%s", err, out)
	}
